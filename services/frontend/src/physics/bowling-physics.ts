/**
 * Main-thread Rapier3D bowling simulation.
 *
 * Extracted from physics-worker.ts to avoid Web Worker WASM hangs.
 * Loads Rapier3D once (async WASM init), then runs synchronous simulation.
 *
...
/** Initialize Rapier WASM. Safe to call multiple times (no-ops after first). */
export async function initRapier(): Promise<boolean> {
  if (rapierReady) return true;
  try {
    await RAPIER.init();
    rapierReady = true;
    return true;
...
const PIN_BELLY_R = 0.058;
const PIN_TAPER_R = 0.036;
const PIN_NECK_COLL_R = 0.023;
const PIN_BASE_HH = 0.015;
const PIN_BELLY_HH = 0.065;
const PIN_TAPER_HH = 0.05;
const PIN_NECK_HH = 0.06;
const PIN_BODY_Y = PIN_H / 2;
...
/**
 * Run a full Rapier3D bowling simulation on the main thread.
 * Must call `await initRapier()` before first use.
 * Runs synchronously (~150-250ms) once WASM is loaded.
 */
export function simulateRapier(params: PhysicsParams): SimulationResult {
  if (!rapierReady) {
  ...
  const ballBodyDesc = RAPIER.RigidBodyDesc.dynamic()
    ...
    .setAngvel({ x: v0 / radius, y: omega0 * 0.5, z: 0 })
    .setCcdEnabled(true)
...
/**
 * High-level async entry point. Initializes Rapier if needed, then runs the simulation.
 * Returns the trajectory frames and summary.
 */
export async function runBowlingSimulation(params: PhysicsParams): Promise<SimulationResult> {
  await initRapier();
  return simulateRapier(params);
}
